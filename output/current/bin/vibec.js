 ...
 */
function mergeConfig(cliOptions) {
  ...
  const envConfig = {
    ...
    noOverwrite: process.env.VIBEC_NO_OVERWRITE === 'true',
  ...
  // Filter out undefined values from environment config
  ...
  // Get config file
  const fileConfig = cliOptions.fileConfig || {};

  // Convert fileConfig.stacks from string to array if it's a string
  if (typeof fileConfig.stacks === 'string') {
    fileConfig.stacks = fileConfig.stacks.split(',');
  }

  // Merge with priority: CLI > env vars > config file > defaults
  const mergedConfig = {
    ...defaults,
    ...fileConfig,
    ...envConfig,
  ...
  if (mergedConfig.retries < 0) {
    log.error('Configuration error: retries must be a non-negative integer');
    process.exit(1);
  }

  if (mergedConfig.pluginTimeout <= 0) {
    log.error('Configuration error: pluginTimeout must be a positive integer');
    process.exit(1);
  }
...
async function main(args) {
  ...
  // Set fileConfig in cliOptions to be included in mergeConfig
  cliOptions.fileConfig = fileConfig;
  ...
  for (const promptFile of promptFiles) {
    ...
    const prompt = await buildPrompt(promptFile.file, promptFile.stack, promptFile.number, options);
    ...
    const pluginContext = {
      ...
      workingDir: path.join(options.output, 'current')
    };
    ...
    if (options.noOverwrite && await checkOverwrite(files, promptFile.number, options.output)) {
    ...
    // Write files
    if (!options.dryRun) {
      await writeFiles(files, promptFile.number, options.output);
    } else {
