...
Options:
  ...
  --iterations=<number>        Number of times a stage should be re-executed to try to resolve test failures (default: 2)
`);
}
...
function processEnvVars(env) {
  ...
  if (env.VIBEC_ITERATIONS) {
    const num = parseInt(env.VIBEC_ITERATIONS, 10);
    if (!isNaN(num) && num >= 0) options.iterations = num;
  }
...
function processConfigFile(config) {
  ...
  if (config.iterations !== undefined) {
    const num = parseInt(config.iterations, 10);
    if (isNaN(num) || num < 0) {
      log.error(`Invalid iterations value in config: ${config.iterations}. Expected a non-negative integer.`);
    } else {
      options.iterations = num;
    }
  }
    ...
    if (arg.startsWith('--')) {
      ...
      if (equalsIndex !== -1) {
        ...
        if (key === 'stacks') {
          ...
          options[key] = num;
        } else if (key === 'retries' || key === 'iterations') {
          const num = parseInt(value, 10);
          if (isNaN(num) || num < 0) {
 ...
 * @param {string} testOutput - Optional test output from previous iteration
 * @returns {Promise<string>} Complete prompt
 */
async function buildPrompt(filePath, workdir, stack, promptNumber, config, testOutput = '') {
  try {
    ...
    // Add test output for iterations after the first
    if (testOutput) {
      contextContent += `\n\n## Test Output from Previous Attempt\n\`\`\`\n${testOutput}\n\`\`\`\n\nPlease fix the issues shown in the test output.`;
    }
...
/**
 * Run tests using the provided test command, capturing stdout and stderr.
 * @param {string|null} testCmd - Test command to execute
 * @param {string} workdir - Working directory
 * @returns {Object} Object containing success status and test output
 */
...
async function main(args) {
  try {
    ...
    for (const [index, promptFile] of promptFiles.entries()) {
      ...
      let iteration = 0;
      let maxIterations = options.iterations || 2;
      let testOutput = '';
      let testsPassed = false;
      
      while (iteration < maxIterations && !testsPassed) {
        if (iteration > 0) {
          log.info(`Iteration ${iteration + 1}/${maxIterations} for prompt ${path.basename(promptFile.file)}`);
        }
        
        // Build prompt with plugin support and test output from previous iteration
        ...
        if (files.length === 0) {
          log.error('No files were generated from LLM response.');
          break;
        }
        
        // Check for overwrites (only on first iteration)
        if (iteration === 0) {
        ...
        // Run tests
        if (options['test-cmd']) {
          const testResult = runTests(options['test-cmd'], options.workdir);
          testsPassed = testResult.success;
          testOutput = testResult.output;
          
          if (testsPassed || iteration >= maxIterations - 1) {
            if (!testsPassed) {
              log.warn(`Tests still failing after ${iteration + 1} iterations.`);
            }
            break;
          }
        } else {
          // If no test command is specified, we're done after the first iteration
          testsPassed = true;
          break;
        }
      ...
      // If tests are still failing after max iterations, fail
      if (options['test-cmd'] && !testsPassed) {
        throw new Error(`Tests failed after ${maxIterations} iterations, stopping execution.`);
      }
    }
