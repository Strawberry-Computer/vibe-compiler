...
// Package version
const VERSION = "0.1.0"; // This should match package.json
...
Options:
  ...
  --stacks=<list>        Comma-separated list of stacks to process (default: core)
  --dry-run              Process without API calls or writing files
  --start=<number>       Start from this prompt number
  --end=<number>         End at this prompt number
...
function parseArgs(argv) {
  ...
  if (options.pluginTimeout <= 0) {
    log.error('--plugin-timeout must be a positive integer');
    process.exit(1);
  }
 ...
 */
async function buildPrompt(filePath, workdir, stack, staticPlugins, outputDir) {
...
/**
 * Process a prompt using an LLM API with retry logic
...
async function processLlm(prompt, options) {
  ...
  for (let attempt = 0; attempt <= options.retries; attempt++) {
    if (attempt > 0) {
      ...
      log.warn(`Retry attempt ${attempt}/${options.retries} after ${delay}ms delay...`);
    ...
    try {
      ...
      lastError = error;
      log.error(`Error calling LLM API (attempt ${attempt + 1}/${options.retries + 1}): ${error.message}`);
...
async function main(argv) {
  ...
  for (const promptFile of promptFiles) {
    ...
    const prompt = await buildPrompt(promptFile.file, options.workdir, promptFile.stack, staticPlugins, options.output);
    
    // Setup output directory paths for this prompt
    const promptFileName = path.basename(promptFile.file, '.md');
    const stackOutputDir = path.join(options.workdir, options.output, 'stacks', promptFile.stack, promptFileName);
    const currentOutputDir = path.join(options.workdir, options.output, 'current');
